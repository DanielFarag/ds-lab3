...
/**
Implement function for BubleSort for doubly linked list ( change pointer )
*/
class Employee {
public:
    int id;
...
public:
    ...
    Node *next;
    Node *prev;
    Node(Employee *_value, Node *_next = NULL, Node *_prev = NULL)
...
public:
    Node *head;
    DoubblyLinkedList() { head = NULL; }

    void append(int x) {
        Employee *emp = new Employee(x);
        Node *el = new Node(emp);
